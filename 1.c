...
void set_data(struct data_shell *datash, char **environ)
{
	int i, char **environ_len = environ;

  /* Count the number of environment variables. */
	for (i = 0; environ_len[i] != NULL; i++)
	{
		environ_len++;
	}
  ...
  /* Allocate memory for the environ array. */
	datash->_environ = malloc((environ_len + 1) * sizeof(char *));
